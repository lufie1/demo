 ...
 */
@WebServlet("/OrderDetailCrudServlet")
public class OrderDetailCrudServlet extends HttpServlet {
	...
	protected void search(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try {
			result = dbtool.SearchOrderDetail(order_id,key, pageIndex, pageSize, sortField, sortOrder);
	...
	protected void save(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		    ...
		    for(int i=0,l=rows.size(); i<l; i++){
		        ...
		        {
		            dbtool.insertOrderDetail(row);
		        }
		        else if (state.equals("removed") || state.equals("deleted"))
		        {
		            dbtool.deleteOrderDetail(id);
		        }
		        else  if(state.equals("modified") || state.equals(""))	//更新：_state为空，或modified
		        {
		            dbtool.updateOrderDetail(row);
		        }
		    }
	...
	protected void get(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		   String id = request.getParameter("id");
		    ...
		    HashMap catagory;
			try {
				catagory = dbtool.GetOrderDetail(id);
